...
class Zoeppritz(ForwardModel):
    ...
    def forward_single_layer(self, a1, b1, rho1, a2, b2, rho2, theta_r):  # 单层的反射系数正演
        """
        ...
        Notes:
            由于要计算反三角函数, 可能得到虚数, 所以速度,密度应为虚数以免得到NAN
        ...
        p = (np.sin(theta_r) / a1).astype(np.complex64)
        if self.ntraces != 1:
            theta_r = np.stack([*[theta_r for _ in range(self.ntraces)]])
        ## clip会丢失虚部数据,失真
        # theta2 = np.arcsin(np.clip(p * a2, -1, 1))  # Trans. angle of P-wave
        # phi1 = np.arcsin(np.clip(p * b1,-1,1))  # Refl. angle of converted S-wave
        # phi2 = np.arcsin(np.clip(p * b2,-1,1))  # Trans. angle of converted S-wave
        ...
        M = np.array(  # shape:[4,4,theta]
            [
                [-sin(theta_r), -cos(phi1), sin(theta2), cos(phi2)],
                [cos(theta_r), -sin(phi1), cos(theta2), -sin(phi2)],
                [
                    2 * rho1 * b1 * sin(phi1) * cos(theta_r),
    ...
    def forward_mutil(self, vp, vs, rho, theta_r, element="PdPu", show=False):  # 多层反射系数正演
        """
        获取所有层的反射系数
        ...
        for i in range(layer):
            ...
            icoef = self.forward_single_layer(a1, b1, rho1, a2, b2, rho2, theta_r)
    ...
    def GenerateGrad_multitrace(self, vp, vs, rho, theta, wav_mtx):
        ...
        #         for m in range(3):
        #             G_mat2[n, i, (m) * self.layers: (m + 1) * self.layers] = (wav_mtx @ G_mat[n, i, m * self.layers: (m + 1) * self.layers].T).T
        #
        return G_mat_
    ...
    def Grad_cal_multitrace(self, vp1, vp2, vs1, vs2, rho1, rho2, Alpha):
        ...
        A_Vp1 = (1 / vp1) * np.array([
            ...
            [2 * (1 - np.cos(2 * Beta)),
             -((np.tan(Beta)) ** 2 - 2) * (vs1 / vp1) * np.sin(2 * Beta),
             (rho2 / rho1) * (vp2 / vp1) * (3 * np.cos(2 * Beta_1) - 2),
             (2 - (np.tan(Beta_1)) ** 2) * (rho2 / rho1) * (vs2 / vp1) * np.sin(2 * Beta_1)],

            [-(vs1 ** 2 / vp1) * np.sin(2 * Alpha),
             2 * vs1 * (1 - np.cos(2 * Beta)),
             ((np.tan(Alpha_1)) ** 2 - 1) * (rho2 / rho1) * (vs2 ** 2 / vp2) * np.sin(2 * Alpha_1),
             (rho2 / rho1) * vs2 * 2 * (np.cos(2 * Beta_1) - 1)]
        ], dtype='float')
        ...
        C_Rho1 = np.array([zero_element, zero_element, zero_element, zero_element], dtype='float')
        # C_Rho1 = C_Rho1.reshape(C_Rho1.shape[0], 1)

        # C_Rho2 = np.array([zero_element, zero_element, zero_element, zero_element], dtype='float')
        # C_Rho2 = C_Rho2.reshape(C_Rho2.shape[0], 1)
